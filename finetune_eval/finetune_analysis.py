...
RUNS = {
	...
	"gpt2" : "finetune_eval/gpt2",
}

CONFIGS_PATH = "C:/Users/timoy/Desktop/syl-gpt-refactor/Configs"
...
for run, folder in RUNS.items():
	for file in os.listdir(folder):
		...
		if tok_key not in tokenizer_index:
			if run == "gpt2":
				# Load HuggingFace GPT-2 tokenizer
				hf_tokenizer = AutoTokenizer.from_pretrained("gpt2")

				COUNT_MIN_ID = 50257 - 29  # 50228 — must match the trainer script


				class HFTokenizerWrapper:
					def __init__(self, tok, count_min_id=COUNT_MIN_ID):
						self.tok = tok
						self.count_min_id = count_min_id

					def decode(self, ids):
						# If it's a single count-token ID, return the <N> format the
						# grapher expects (e.g. ID 50230 → count 3 → "<3>")
						if len(ids) == 1 and ids[0] >= self.count_min_id:
							count = ids[0] - self.count_min_id + 1
							return f"<{count}>"
				...
				tokenizer_index[tok_key] = HFTokenizerWrapper(hf_tokenizer)
