...
# ---- STEPS: 128 → 3500 (step 128) ----
STEPS=()
for ((s=128; s<=3500; s+=128)); do
  STEPS+=("$s")
done
...
declare -A WEIGHTS_FILES=(
  [eng_uni]="$WEIGHTS_BASE/eng_uni_%s_good.pth"
  [eng_bpe]="$WEIGHTS_BASE/eng_bpe_%s_good.pth"
  [span_syl]="$WEIGHTS_BASE/span_syl_%s_good.pth"
  [span_uni]="$WEIGHTS_BASE/span_uni_%s_good.pth"
  [span_bpe]="$WEIGHTS_BASE/span_bpe_%s_good.pth"
)
...
for lang in "${LANGS[@]}"; do
  ...
  for paradigm in "${PARADIGMS[@]}"; do
    ...
    for step in "${STEPS[@]}"; do
      ...
      WEIGHTS_PATH=""
      WEIGHTS_PATH=$(printf "${WEIGHTS_FILES[$KEY]}" "$step")

      if [[ -n "$WEIGHTS_PATH" && ! -f "$WEIGHTS_PATH" ]]; then
        echo "✗ Missing pretrained weights: $WEIGHTS_PATH — skipping"
        continue
      fi
      ...
      for prefix in "${TASK_PREFIXES[@]}"; do
        for base_task in "${TASKS[@]}"; do
          ...
          for cv in "${CV_COUNTERS[@]}"; do
            WEIGHT_FILE="${FINETUNED_BASE}/${TASK}_${lang}_${paradigm}_step${step}_${cv}_finetuned.pth"
