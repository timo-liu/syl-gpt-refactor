...
args.input_bin = f"{cli_args.task}_{cli_args.language}_gpt2_train_{cli_args.cross_val_counter}_*.bin"
args.input_val_bin = f"{cli_args.task}_{cli_args.language}_gpt2_val_{cli_args.cross_val_counter}_*.bin"
args.batch_size = 8
...
optimizer4 = torch.optim.Adam(scalar_params, lr=0.0004, betas=(0.8, 0.95), fused=True) # note that this learning rate is neither sensitive nor tuned
	...
	if (last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0)):
		...
		for _ in range(val_steps):
			with torch.no_grad():
				...
				val_loss += model(input_ids=x_val, labels=y_val).loss
...
if master_process:
	...
	START_ID = 50257 - 30
	COUNT_MIN_ID = 50257 - 30
	COUNT_MAX_ID = 50257 - 1
	pattern = os.path.join(cli_args.data_path,
	...
	out_file = os.path.join(
		out_dir,
		f"{cli_args.task}_{cli_args.language}_gpt2_{cli_args.cross_val_counter}.txt"
	)

	meta_file = os.path.join(
		out_dir,
		f"{cli_args.task}_{cli_args.language}_gpt2_{cli_args.cross_val_counter}_meta.txt"
	)

	if cli_args.task in ["syllables", "word"]:

		# split right after the count token (<k>)
		...
		with open(meta_file, "w") as meta:
			for s in splits:
				...
				inp = torch.tensor(s[1:-1]).unsqueeze(0).to("cuda")
				target = int(s[-1])
				...
				with torch.inference_mode():
					...
					masked_logits[:COUNT_MIN_ID] = float('-inf')
					masked_logits[COUNT_MAX_ID + 1:] = float('-inf')
